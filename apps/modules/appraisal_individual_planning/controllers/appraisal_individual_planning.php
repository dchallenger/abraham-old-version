...
*/
        ...
        switch( $status_id )
        {
            ...
            case 11: //for immediate supervisors review.
                ...
                $this->db->update('performance_planning_approver', array('performance_status_id' => 11, 'modified_on' => date('Y-m-d')), $where);
                ...
                // email to immediate for final review of the planning
                $employee = $appraisee->created_by;
                $sendtargetsettings['immediate'] = $appraisee->immediate;

                $target_settings_send_template = $this->db->get_where( 'system_template', array( 'code' => 'PERFORMANCE-TARGET-SETTINGS-SEND-IMMEDIATE-FROM-APPRAISEE') )->row_array();
                $msg = $this->parser->parse_string($target_settings_send_template['body'], $sendtargetsettings, TRUE); 
                $subject = $this->parser->parse_string($target_settings_send_template['subject'], $sendtargetsettings, TRUE); 
            ...
            case 4: //for immediate supervisor approver approval
                ...
                foreach(  $approvers  as $index => $approver )
                {
                    ...
                    if( $index == 0 )
                    {
                        ...
                        // email to immediate to inform that planning has been approved by the approver
                        $employee = $appraisee->created_by;
                ...
                // email to appraisee to inform that planning has been approved by the approver
                $employee = $_POST['user_id'];
                $sendtargetsettings['recepient'] = $appraisee->alias;
