...
MySQL - 5.5.55-0ubuntu0.14.04.1 : Database - emplopad.uat
*********************************************************************
*/
...
/*!50001 CREATE TABLE  `applicant_details`(
 ...
 `present_address_no` text ,
 `present_address_street` text ,
 `present_address_village` text ,
 `present_address_barangay` text ,
 `present_address_town` text ,
 `present_address_city_town` text ,
 `present_address_province` text ,
 `present_address_country` text ,
 `present_ddress` mediumtext ,
 `present_province` mediumtext ,
 `phone` text ,
 `mobile` text ,
 `birth_place` text ,
 `height` text ,
...
/*!50001 CREATE TABLE  `dashboard_feeds`(
 `id` int(11) unsigned ,
 `user_id` int(11) ,
 `message_type` enum('Admin','Announcement','Birthday','Comment','Company News','Feedback','Partners','Personnel','System','Time Record','Code of Conduct','Movement','Signatories','Clearance') ,
 `class` varchar(13) ,
 `display_name` varchar(64) ,
 `feed_content` text ,
...
/*!50001 DROP VIEW IF EXISTS `payroll_journal_voucher_osi` */;
/*!50001 DROP TABLE IF EXISTS `payroll_journal_voucher_osi` */;

/*!50001 CREATE TABLE  `payroll_journal_voucher_osi`(
 `arrangement` varchar(3) ,
 `payroll_date` date ,
 `date_from` date ,
 `date_to` date ,
...
/*!50001 CREATE TABLE  `report_recruitment_funnel`(
 `poc` varchar(64) ,
 `status` varchar(64) ,
 `no_of_headcount_required` int(1) ,
 `prf` varchar(16) ,
 `date_opened` date ,
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `approver_class_department` AS (select `up`.`company_id` AS `company_id`,`ud`.`department_id` AS `department_id`,`ud`.`department` AS `department` from (`ww_users_department` `ud` join `users_profile` `up` on((`ud`.`department_id` = `up`.`department_id`))) group by 1,2 order by 1,3) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `approver_position_users` AS (select `a`.`user_id` AS `user_id`,`a`.`full_name` AS `full_name`,`b`.`company_id` AS `company_id`,`b`.`department_id` AS `department_id`,`b`.`position_id` AS `position_id` from (`users` `a` join `users_profile` `b` on(((`b`.`user_id` = `a`.`user_id`) and (`a`.`active` = 1))))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `payroll_journal_voucher_extended` AS select `pct`.`payroll_date` AS `payroll_date`,`pp`.`date_from` AS `date_from`,`pp`.`date_to` AS `date_to`,`uc`.`company` AS `company`,`uc`.`address` AS `address`,`pct`.`branch_id` AS `branch_id`,if((ifnull(`pas`.`category_value`,'') = ''),`ub`.`branch_code`,`pas`.`category_value`) AS `branch`,if((ifnull(`pas`.`account_code`,'') = ''),convert(`pa`.`account_code` using utf8),`pas`.`account_code`) AS `account`,if((ifnull(`pas`.`account_code`,'') = ''),convert(`pa`.`account_name` using utf8),`pas`.`account_sub`) AS `description`,if((ifnull(`pas`.`account_code`,'') = ''),if((`pa`.`department_breakdown` = 0),'000-000-000',`ud`.`department_code`),`pas`.`account_sub_code`) AS `sub_account`,'' AS `project`,'' AS `project_task`,'' AS `ref_number`,1 AS `quantity`,'' AS `uom`,sum(if((if((`pa`.`department_breakdown` = 0),1,((`pct`.`department_id` = `ud`.`department_id`) and (`pa`.`department_breakdown` = `ud`.`category_id`))) and find_in_set(`pa`.`account_id`,`pt`.`debit_account`)),(if(((`pt`.`transaction_type_id` = 5) and (`ptc`.`government_mandated` = 0)),-(1),1) * aes_decrypt(`pct`.`amount`,`encryption_key`())),0)) AS `debit_amount`,sum(if((if((`pa`.`department_breakdown` = 0),1,((`pct`.`department_id` = `ud`.`department_id`) and (`ud`.`category_id` = `pa`.`department_breakdown`))) and find_in_set(`pa`.`account_id`,`pt`.`credit_account`)),aes_decrypt(`pct`.`amount`,`encryption_key`()),0)) AS `credit_amount`,`pp`.`remarks` AS `transaction_description`,'FALSE' AS `non_billable` from ((((((((((`ww_payroll_closed_transaction` `pct` join `ww_payroll_account` `pa`) left join `ww_payroll_period` `pp` on((`pp`.`payroll_period_id` = `pct`.`period_id`))) left join `ww_users_company` `uc` on((`uc`.`company_id` = `pct`.`company_id`))) left join `ww_users_branch` `ub` on((`ub`.`branch_id` = `pct`.`branch_id`))) left join `ww_users_department` `ud` on((`ud`.`department_id` = `pct`.`department_id`))) left join `ww_payroll_transaction` `pt` on((`pt`.`transaction_id` = `pct`.`transaction_id`))) left join `ww_payroll_transaction_class` `ptc` on((`ptc`.`transaction_class_id` = `pt`.`transaction_class_id`))) left join `ww_payroll_account_type` `pat` on((`pat`.`account_type_id` = `pa`.`account_type_id`))) left join `ww_users` `u` on(((`u`.`active` = 1) and (`pct`.`employee_id` = `u`.`user_id`)))) left join `ww_payroll_account_sub` `pas` on(((`pas`.`account_id` = `pa`.`account_id`) and (`pas`.`category_val_id` = `ub`.`branch_id`) and if((`pas`.`sub_category_id` = 0),1,(`pas`.`sub_category_id` = `ud`.`department_id`))))) where (`pa`.`deleted` = 0) group by `pct`.`payroll_date`,`pct`.`branch_id`,`pa`.`account_code`,if((`pa`.`department_breakdown` = 0),1,`ud`.`department_id`) having ((`debit_amount` > 0) or (`credit_amount` > 0)) order by `pct`.`payroll_date`,`pct`.`branch_id`,`pa`.`account_code`,`ud`.`department_code` */;

/*View structure for view payroll_journal_voucher_osi */

/*!50001 DROP TABLE IF EXISTS `payroll_journal_voucher_osi` */;
/*!50001 DROP VIEW IF EXISTS `payroll_journal_voucher_osi` */;

/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `payroll_journal_voucher_osi` AS select ifnull(`pa`.`arrangement`,`pb`.`arrangement`) AS `arrangement`,`pct`.`payroll_date` AS `payroll_date`,`pp`.`date_from` AS `date_from`,`pp`.`date_to` AS `date_to`,`pct`.`company_id` AS `company_id`,`uc`.`company` AS `company`,ifnull(ifnull(`pas`.`account_sub_code`,convert(`pa`.`account_code` using utf8)),ifnull(`pbs`.`account_sub_code`,convert(`pb`.`account_code` using utf8))) AS `account_number`,ifnull(ifnull(`pas`.`account_sub`,convert(`pa`.`account_name` using utf8)),ifnull(`pbs`.`account_sub`,convert(`pb`.`account_name` using utf8))) AS `account_title`,ifnull(if(((`pbs`.`category_value` = '') or isnull(`pbs`.`category_value`)),ifnull(`pas`.`category_value`,''),`pbs`.`category_value`),'') AS `dept`,`pt`.`transaction_label` AS `particulars`,if((`pt`.`debit_account_id` > 0),if((`pct`.`transaction_code` in ('ABSENCES_ADJ','LWOP_ADJ')),(-(1) * round(sum(aes_decrypt(`pct`.`amount`,`encryption_key`())),2)),round(sum(aes_decrypt(`pct`.`amount`,`encryption_key`())),2)),0) AS `Dr`,if((`pt`.`credit_account_id` > 0),if((isnull(`pb`.`cond`) or (`pb`.`cond` = '')),round(sum(aes_decrypt(`pct`.`amount`,`encryption_key`())),2),(`get_additional_amount`(`pb`.`cond`,`pct`.`payroll_date`,`pct`.`company_id`,1) + round(sum(aes_decrypt(`pct`.`amount`,`encryption_key`())),2))),0) AS `Cr`,`pt`.`transaction_type_id` AS `transaction_type_id` from ((((((((`ww_payroll_closed_transaction` `pct` left join `ww_payroll_transaction` `pt` on((`pct`.`transaction_id` = `pt`.`transaction_id`))) left join `ww_payroll_transaction_class` `ptc` on((`pt`.`transaction_class_id` = `ptc`.`transaction_class_id`))) left join `ww_payroll_account` `pa` on(((`pt`.`debit_account_id` = `pa`.`account_id`) and (`pa`.`deleted` = 0)))) left join `ww_payroll_account_sub` `pas` on(((`pa`.`account_id` = `pas`.`account_id`) and (`pas`.`deleted` = 0) and if((`pct`.`section_id` > 0),((`pas`.`category_val_id` = `pct`.`section_id`) and (`pas`.`category_id` = 6)),((`pas`.`category_val_id` = `pct`.`department_id`) and (`pas`.`category_id` = 2)))))) left join `ww_payroll_account` `pb` on(((`pt`.`credit_account_id` = `pb`.`account_id`) and (`pb`.`deleted` = 0)))) left join `ww_payroll_account_sub` `pbs` on(((`pb`.`account_id` = `pbs`.`account_id`) and (`pbs`.`deleted` = 0) and if((`pct`.`section_id` > 0),((`pbs`.`category_val_id` = `pct`.`section_id`) and (`pbs`.`category_id` = 6)),((`pbs`.`category_val_id` = `pct`.`department_id`) and (`pbs`.`category_id` = 2)))))) left join `ww_payroll_period` `pp` on((`pct`.`period_id` = `pp`.`payroll_period_id`))) left join `ww_users_company` `uc` on((`pct`.`company_id` = `uc`.`company_id`))) where ((`pct`.`deleted` = 0) and (`pt`.`deleted` = 0)) group by `pct`.`payroll_date`,`pct`.`company_id`,ifnull(ifnull(`pas`.`account_sub_code`,convert(`pa`.`account_code` using utf8)),ifnull(`pbs`.`account_sub_code`,convert(`pb`.`account_code` using utf8))),ifnull(ifnull(`pas`.`account_sub`,convert(`pa`.`account_name` using utf8)),ifnull(`pbs`.`account_sub`,convert(`pb`.`account_name` using utf8))),ifnull(ifnull(`pas`.`category_value`,''),ifnull(`pbs`.`category_value`,'')),`pt`.`transaction_label` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `payroll_pagibig_to_disk` AS select `u`.`user_id` AS `user_id`,`p`.`id_number` AS `id_number`,`up`.`title` AS `title`,`up`.`lastname` AS `lastname`,`up`.`firstname` AS `firstname`,`up`.`middlename` AS `middlename`,`up`.`suffix` AS `suffix`,`u`.`full_name` AS `full_name`,`uc`.`company` AS `company`,`uc`.`company_id` AS `company_id`,`b`.`hdmf_branch_code` AS `bcode`,if(((`b`.`hdmf_branch_code` <> '00') and (`b`.`hdmf_branch_code` is not null)),`b`.`hdmf_branch_code`,ifnull(`uc`.`hdmf_branch_code`,'00')) AS `pagibig_branch_code`,`uc`.`tin` AS `tin`,`uc`.`sss` AS `co_sss`,`uc`.`phic` AS `co_phic`,`uc`.`hdmf` AS `co_hdmf`,`uc`.`address` AS `co_address`,`uc`.`zipcode` AS `zipcode`,`getcompany_contact`(`pp`.`company_id`,'phone') AS `contact_no`,`ud`.`department_id` AS `department_id`,`ud`.`department` AS `department`,`up`.`branch_id` AS `branch_id`,`b`.`branch` AS `branch`,year(`pct`.`payroll_date`) AS `year`,month(`pct`.`payroll_date`) AS `month`,`pct`.`payroll_date` AS `payroll_date`,`up`.`birth_date` AS `birth_date`,`pp`.`sss_no` AS `sss_no`,`pp`.`phic_no` AS `phic_no`,`pp`.`hdmf_no` AS `hdmf_no`,`pp`.`tin` AS `tin_no`,`p`.`effectivity_date` AS `hired_date`,`p`.`resigned_date` AS `resigned_date`,`get_sbr_no`(`pct`.`employee_id`,`pct`.`payroll_date`,'HDMF_EMP') AS `sbr_no_hdmf`,`get_sbr_no`(`pct`.`employee_id`,`pct`.`payroll_date`,'PHIC_EMP') AS `sbr_no_phic`,`get_sbr_no`(`pct`.`employee_id`,`pct`.`payroll_date`,'SSS_EMP') AS `sbr_no_sss`,`get_sbr_date`(`pct`.`employee_id`,`pct`.`payroll_date`,'HDMF_EMP') AS `sbr_date_hdmf`,`get_sbr_date`(`pct`.`employee_id`,`pct`.`payroll_date`,'PHIC_EMP') AS `sbr_date_phic`,`get_sbr_date`(`pct`.`employee_id`,`pct`.`payroll_date`,'SSS_EMP') AS `sbr_date_sss`,(case when ((year(`p`.`effectivity_date`) = year(`pct`.`payroll_date`)) and (month(`p`.`effectivity_date`) = month(`pct`.`payroll_date`))) then '1' when ((year(`p`.`resigned_date`) = year(`pct`.`payroll_date`)) and (month(`p`.`resigned_date`) = month(`pct`.`payroll_date`))) then '2' else 'N' end) AS `govt_status`,sum(((case when (`pct`.`transaction_code` = 'SSS_EMP') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `sss_emp`,sum(((case when (`pct`.`transaction_code` = 'SSS_COM') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `sss_com`,sum(((case when (`pct`.`transaction_code` = 'SSS_ECC') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `sss_ecc`,sum(((case when (`pct`.`transaction_code` = 'HDMF_EMP') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `hdmf_emp`,sum(((case when (`pct`.`transaction_code` = 'HDMF_COM') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `hdmf_com`,sum(((case when (`pct`.`transaction_code` = 'PHIC_EMP') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `phic_emp`,sum(((case when (`pct`.`transaction_code` = 'PHIC_COM') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `phic_com`,sum(((case when (`pct`.`transaction_code` = 'PagIbigAdd') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `PagIbigAdd`,concat('DT',rpad(`pp`.`hdmf_no`,12,' '),rpad(`p`.`id_number`,15,' '),(case when (locate('ñ',`up`.`lastname`) <> 'false') then rpad(replace(`up`.`lastname`,' *',''),30,' ') else rpad(replace(`up`.`lastname`,' *',''),30,' ') end),(case when (locate('ñ',`up`.`firstname`) <> 'false') then rpad(`up`.`firstname`,30,' ') else rpad(`up`.`firstname`,30,' ') end),(case when ((`up`.`middlename` <> '') and (`up`.`middlename` is not null)) then rpad(`up`.`middlename`,30,' ') else rpad('',30,' ') end),rpad(round((sum(((case when (`pct`.`transaction_code` = 'HDMF_EMP') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) + sum(((case when (`pct`.`transaction_code` = 'PagIbigAdd') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`())))),2),13,' '),rpad(round(sum(((case when (`pct`.`transaction_code` = 'HDMF_COM') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))),2),13,' '),rpad(ifnull(replace(`pp`.`tin`,'-',''),''),15,' '),rpad(if((`up`.`birth_date` is not null),date_format(`up`.`birth_date`,'%Y%m%d'),''),8,'0')) AS `record`,now() AS `document_date` from (((((((`ww_payroll_current_transaction` `pct` join `ww_payroll_partners` `pp` on((`pp`.`user_id` = `pct`.`employee_id`))) join `ww_partners` `p` on((`pct`.`employee_id` = `p`.`user_id`))) join `ww_users` `u` on((`pct`.`employee_id` = `u`.`user_id`))) join `ww_users_profile` `up` on((`pct`.`employee_id` = `up`.`user_id`))) join `ww_users_company` `uc` on((`uc`.`company_id` = `pp`.`company_id`))) join `ww_users_department` `ud` on((`up`.`department_id` = `ud`.`department_id`))) left join `ww_users_branch` `b` on((`b`.`branch_id` = `up`.`branch_id`))) where ((`pct`.`transaction_code` in ('SSS_EMP','SSS_COM','SSS_ECC','HDMF_EMP','HDMF_COM','PHIC_EMP','PHIC_COM','PagIbigAdd')) and (`pct`.`deleted` = 0) and (`pct`.`on_hold` = 0)) group by `u`.`user_id`,`p`.`id_number`,`u`.`full_name`,`uc`.`company`,`uc`.`company_id`,`uc`.`sss`,`uc`.`phic`,`uc`.`hdmf`,`uc`.`address`,`ud`.`department_id`,`ud`.`department`,year(`pct`.`payroll_date`),month(`pct`.`payroll_date`),`up`.`birth_date`,`pp`.`sss_no`,`pp`.`phic_no`,`pp`.`hdmf_no` union select `u`.`user_id` AS `user_id`,`p`.`id_number` AS `id_number`,`up`.`title` AS `title`,`up`.`lastname` AS `lastname`,`up`.`firstname` AS `firstname`,`up`.`middlename` AS `middlename`,`up`.`suffix` AS `suffix`,`u`.`full_name` AS `full_name`,`uc`.`company` AS `company`,`uc`.`company_id` AS `company_id`,`b`.`hdmf_branch_code` AS `bcode`,if(((`b`.`hdmf_branch_code` <> '00') and (`b`.`hdmf_branch_code` is not null)),`b`.`hdmf_branch_code`,ifnull(`uc`.`hdmf_branch_code`,'00')) AS `pagibig_branch_code`,`uc`.`tin` AS `tin`,`uc`.`sss` AS `co_sss`,`uc`.`phic` AS `co_phic`,`uc`.`hdmf` AS `co_hdmf`,`uc`.`address` AS `co_address`,`uc`.`zipcode` AS `zipcode`,`getcompany_contact`(`pp`.`company_id`,'phone') AS `contact_no`,`ud`.`department_id` AS `department_id`,`ud`.`department` AS `department`,`up`.`branch_id` AS `branch_id`,`b`.`branch` AS `branch`,year(`pct`.`payroll_date`) AS `year`,month(`pct`.`payroll_date`) AS `month`,`pct`.`payroll_date` AS `payroll_date`,`up`.`birth_date` AS `birth_date`,`pp`.`sss_no` AS `sss_no`,`pp`.`phic_no` AS `phic_no`,`pp`.`hdmf_no` AS `hdmf_no`,`pp`.`tin` AS `tin_no`,`p`.`effectivity_date` AS `hired_date`,`p`.`resigned_date` AS `resigned_date`,`get_sbr_no`(`pct`.`employee_id`,`pct`.`payroll_date`,'HDMF_EMP') AS `sbr_no_hdmf`,`get_sbr_no`(`pct`.`employee_id`,`pct`.`payroll_date`,'PHIC_EMP') AS `sbr_no_phic`,`get_sbr_no`(`pct`.`employee_id`,`pct`.`payroll_date`,'SSS_EMP') AS `sbr_no_sss`,`get_sbr_date`(`pct`.`employee_id`,`pct`.`payroll_date`,'HDMF_EMP') AS `sbr_date_hdmf`,`get_sbr_date`(`pct`.`employee_id`,`pct`.`payroll_date`,'PHIC_EMP') AS `sbr_date_phic`,`get_sbr_date`(`pct`.`employee_id`,`pct`.`payroll_date`,'SSS_EMP') AS `sbr_date_sss`,(case when ((year(`p`.`effectivity_date`) = year(`pct`.`payroll_date`)) and (month(`p`.`effectivity_date`) = month(`pct`.`payroll_date`))) then '1' when ((year(`p`.`resigned_date`) = year(`pct`.`payroll_date`)) and (month(`p`.`resigned_date`) = month(`pct`.`payroll_date`))) then '2' else 'N' end) AS `govt_status`,sum(((case when (`pct`.`transaction_code` = 'SSS_EMP') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `sss_emp`,sum(((case when (`pct`.`transaction_code` = 'SSS_COM') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `sss_com`,sum(((case when (`pct`.`transaction_code` = 'SSS_ECC') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `sss_ecc`,sum(((case when (`pct`.`transaction_code` = 'HDMF_EMP') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `hdmf_emp`,sum(((case when (`pct`.`transaction_code` = 'HDMF_COM') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `hdmf_com`,sum(((case when (`pct`.`transaction_code` = 'PHIC_EMP') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `phic_emp`,sum(((case when (`pct`.`transaction_code` = 'PHIC_COM') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `phic_com`,sum(((case when (`pct`.`transaction_code` = 'PagIbigAdd') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) AS `PagIbigAdd`,concat('DT',rpad(`pp`.`hdmf_no`,12,' '),rpad(`p`.`id_number`,15,' '),(case when (locate('ñ',`up`.`lastname`) <> 'false') then rpad(replace(`up`.`lastname`,' *',''),30,' ') else rpad(replace(`up`.`lastname`,' *',''),30,' ') end),(case when (locate('ñ',`up`.`firstname`) <> 'false') then rpad(`up`.`firstname`,30,' ') else rpad(`up`.`firstname`,30,' ') end),(case when ((`up`.`middlename` <> '') and (`up`.`middlename` is not null)) then rpad(`up`.`middlename`,30,' ') else rpad('',30,' ') end),rpad(round((sum(((case when (`pct`.`transaction_code` = 'HDMF_EMP') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))) + sum(((case when (`pct`.`transaction_code` = 'PagIbigAdd') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`())))),2),13,' '),rpad(round(sum(((case when (`pct`.`transaction_code` = 'HDMF_COM') then 1 else 0 end) * aes_decrypt(`pct`.`amount`,`encryption_key`()))),2),13,' '),rpad(ifnull(replace(`pp`.`tin`,'-',''),''),15,' '),rpad(if((`up`.`birth_date` is not null),date_format(`up`.`birth_date`,'%Y%m%d'),''),8,'0')) AS `record`,now() AS `document_date` from (((((((`ww_payroll_closed_transaction` `pct` join `ww_payroll_partners` `pp` on((`pp`.`user_id` = `pct`.`employee_id`))) join `ww_partners` `p` on((`pct`.`employee_id` = `p`.`user_id`))) join `ww_users` `u` on((`pct`.`employee_id` = `u`.`user_id`))) join `ww_users_profile` `up` on((`pct`.`employee_id` = `up`.`user_id`))) join `ww_users_company` `uc` on((`uc`.`company_id` = `pp`.`company_id`))) join `ww_users_department` `ud` on((`up`.`department_id` = `ud`.`department_id`))) left join `ww_users_branch` `b` on((`b`.`branch_id` = `up`.`branch_id`))) where ((`pct`.`transaction_code` in ('SSS_EMP','SSS_COM','SSS_ECC','HDMF_EMP','HDMF_COM','PHIC_EMP','PHIC_COM','PagIbigAdd')) and (`pct`.`deleted` = 0) and (`pct`.`on_hold` = 0)) group by `u`.`user_id`,`p`.`id_number`,`u`.`full_name`,`uc`.`company`,`uc`.`company_id`,`uc`.`sss`,`uc`.`phic`,`uc`.`hdmf`,`uc`.`address`,`ud`.`department_id`,`ud`.`department`,year(`pct`.`payroll_date`),month(`pct`.`payroll_date`),`up`.`birth_date`,`pp`.`sss_no`,`pp`.`phic_no`,`pp`.`hdmf_no` */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `payroll_payslip_riofil` AS select `u`.`user_id` AS `employee`,`p`.`status` AS `status`,`upos`.`position` AS `position`,`pp`.`company_id` AS `company_id`,`uc`.`company` AS `company`,`uc`.`address` AS `address`,`uc`.`city` AS `city`,`u`.`user_id` AS `user_id`,`p`.`id_number` AS `id_number`,`u`.`full_name` AS `full_name`,`up`.`department_id` AS `department_id`,`d`.`department_code` AS `department_code`,`uproj`.`project_id` AS `project_id`,`uproj`.`project_code` AS `project_code`,`uproj`.`project` AS `project`,`prt`.`payroll_rate_type_id` AS `payroll_rate_type_id`,`prt`.`payroll_rate_type` AS `payroll_rate_type`,0 AS `adjustment`,0 AS `other_taxable`,(((case when (`pct`.`transaction_code` = 'ABSENCES') then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) + (case when (`pct`.`transaction_code` = 'DEDUCTION_LATE') then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end)) + (case when (`pct`.`transaction_code` = 'DEDUCTION_UNDERTIME') then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end)) AS `absent_tardy`,0 AS `sss`,0 AS `philhealth`,0 AS `pag_ibig`,0 AS `nontax_income`,(case when (`pct`.`transaction_code` = 'REGND') then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `nd`,(case when (`pct`.`transaction_code` = 'REGOT') then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `reg`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_x8`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `sp`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `sp_x8`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_sp`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_sp_x8`,round((((aes_decrypt(`pp`.`salary`,`encryption_key`()) * 12) / `pp`.`total_year_days`) / 8),2) AS `hourly_rate`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `legal`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `legal_x8`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_legal`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_leg_x8`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'REGOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `nd_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `reg_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_x8_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `sp_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `sp_x8_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_sp_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_sp_x8_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `legal_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `legal_x8_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_legal_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_leg_x8_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `nd_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'REGOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `reg_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT_EXCESS')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_x8_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `sp_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT_EXCESS')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `sp_x8_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_sp_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT_EXCESS')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_sp_x8_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `legal_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT_EXCESS')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `legal_x8_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_legal_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT_EXCESS')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_leg_x8_hrs`,0 AS `health_card`,0 AS `other_deduction_one`,0 AS `other_deduction_two`,0 AS `other_deduction_three`,0 AS `sss_sal_loan_payments`,0 AS `sss_cal_loan_payments`,0 AS `hdmf_sal_loan_payments`,0 AS `hdmf_cal_loan_payments`,0 AS `company_loan_payments`,0 AS `sss_sal_loan_balance`,0 AS `sss_cal_loan_balance`,0 AS `hdmf_sal_loan_balance`,0 AS `hdmf_cal_loan_balance`,0 AS `tax_status`,0 AS `ytd_sss`,0 AS `ytd_philhealth`,0 AS `ytd_pag_ibig`,`pp`.`tin` AS `tin`,`pt`.`transaction_label` AS `transaction_label`,`ptc`.`transaction_class_code` AS `transaction_class_code`,`pt`.`transaction_code` AS `transaction_code`,(case when (`pct`.`transaction_code` in ('ABSENCES','ABSENCES_ADJ','LWOP','LWOP_ADJ')) then concat(aes_decrypt(`pct`.`quantity`,`encryption_key`()),' (',`getabsent`(`t`.`payroll_date`,`u`.`user_id`,`pct`.`transaction_code`),') ') else aes_decrypt(`pct`.`quantity`,`encryption_key`()) end) AS `qty`,round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) AS `amount`,`pct`.`transaction_type_id` AS `transaction_type_id`,(case when (`pct`.`transaction_type_id` in (1,2,6,7,8)) then 'Earnings' when (`pct`.`transaction_type_id` in (3,4,5)) then 'Deductions' when (`pct`.`transaction_code` = 'NETPAY') then 'Netpay' else '' end) AS `group`,(case when ((`ptc`.`transaction_class_code` in ('LOAN_AMORTIZATION','LOAN_INTEREST')) and (`ptc`.`government_mandated` = 0)) then 'Loan' when ((`ptc`.`government_mandated` = 1) or (`ptc`.`transaction_class_code` = 'WHTAX')) then 'government' when (`ptc`.`transaction_class_code` = 'OVERTIME') then 'overtime' when ((`ptc`.`transaction_class_code` in ('ABSENCES','DEDUCTION_LATE','DEDUCTION_UNDERTIME')) or (`pt`.`transaction_code` = 'LWOP')) then 'attnd_ded' when (`pt`.`transaction_type_id` = 8) then 'Bonus' when (`pt`.`transaction_type_id` in (2,6)) then 'Benefits' when ((`pt`.`transaction_type_id` = 1) and (`pt`.`transaction_id` <> 1)) then 'Earnings' when (`pt`.`transaction_id` = 1) then 'salary' when (`pt`.`transaction_code` = 'NETPAY') then 'Netpay' else (case when (`pt`.`transaction_type_id` in (3,4,5)) then 'Oth_ded' when (`pt`.`transaction_type_id` in (1,2,6,7,8)) then 'Oth_inc' else '' end) end) AS `type`,(case when (`ptc`.`transaction_class_code` = 'LOAN_AMORTIZATION') then `pct`.`record_id` else '' end) AS `record_id`,(case when (`ptc`.`transaction_class_code` = 'LOAN_AMORTIZATION') then round(aes_decrypt(`pl`.`beginning_balance`,`encryption_key`()),2) else 0 end) AS `beginning_balance`,(case when (`ptc`.`transaction_class_code` = 'LOAN_AMORTIZATION') then round(aes_decrypt(`pl`.`running_balance`,`encryption_key`()),2) else 0 end) AS `running_balance`,`t`.`payroll_date` AS `payroll_date`,`t`.`date_from` AS `date_from`,`t`.`date_to` AS `date_to`,concat(date_format(if((`ptc`.`transaction_class_code` = 'NETPAY'),`t`.`payroll_date`,`t`.`date_from`),'%Y-%m'),'-',date_format(`t`.`date_from`,'%d')) AS `date_from_applicable`,date_format(if((`ptc`.`transaction_class_code` = 'NETPAY'),last_day(`t`.`payroll_date`),`t`.`date_to`),'%Y-%m-%d') AS `date_to_applicable`,`tc`.`taxcode` AS `taxcode`,`getcompany_contact`(`pct`.`company_id`,'Phone') AS `phone_no`,`getcompany_contact`(`pct`.`company_id`,'Fax') AS `fax_no` from (((((((((((((((`ww_payroll_current_transaction` `pct` join `ww_users` `u` on((`pct`.`employee_id` = `u`.`user_id`))) join `ww_partners` `p` on((`u`.`user_id` = `p`.`user_id`))) join `ww_partners_personal` `ppe` on((`p`.`partner_id` = `ppe`.`partner_id`))) join `ww_taxcode` `tc` on((`tc`.`taxcode_id` = `ppe`.`key_value`))) join `ww_users_profile` `up` on((`u`.`user_id` = `up`.`user_id`))) join `ww_users_position` `upos` on((`up`.`position_id` = `upos`.`position_id`))) join `ww_payroll_partners` `pp` on((`pp`.`user_id` = `u`.`user_id`))) join `ww_users_department` `d` on((`d`.`department_id` = `up`.`department_id`))) join `ww_users_company` `uc` on((`uc`.`company_id` = `pp`.`company_id`))) left join `ww_payroll_period` `t` on((`t`.`payroll_period_id` = `pct`.`period_id`))) left join `ww_payroll_transaction` `pt` on((`pt`.`transaction_id` = `pct`.`transaction_id`))) left join `ww_payroll_transaction_class` `ptc` on((`pt`.`transaction_class_id` = `ptc`.`transaction_class_id`))) left join `ww_payroll_partners_loan` `pl` on((`pl`.`partner_loan_id` = `pct`.`record_id`))) left join `ww_users_project` `uproj` on((`pct`.`project_id` = `uproj`.`project_id`))) left join `ww_payroll_rate_type` `prt` on((`pct`.`payroll_rate_type_id` = `prt`.`payroll_rate_type_id`))) where ((`pct`.`on_hold` = 0) and (`ppe`.`key` = 'taxcode')) union all select `u`.`user_id` AS `employee`,`p`.`status` AS `status`,`upos`.`position` AS `position`,`pp`.`company_id` AS `company_id`,`uc`.`company` AS `company`,`uc`.`address` AS `address`,`uc`.`city` AS `city`,`u`.`user_id` AS `user_id`,`p`.`id_number` AS `id_number`,`u`.`full_name` AS `full_name`,`up`.`department_id` AS `department_id`,`d`.`department_code` AS `department_code`,`uproj`.`project_id` AS `project_id`,`uproj`.`project_code` AS `project_code`,`uproj`.`project` AS `project`,`prt`.`payroll_rate_type_id` AS `payroll_rate_type_id`,`prt`.`payroll_rate_type` AS `payroll_rate_type`,0 AS `adjustment`,0 AS `other_taxable`,(((case when (`pct`.`transaction_code` = 'ABSENCES') then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) + (case when (`pct`.`transaction_code` = 'DEDUCTION_LATE') then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end)) + (case when (`pct`.`transaction_code` = 'DEDUCTION_UNDERTIME') then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end)) AS `absent_tardy`,0 AS `sss`,0 AS `philhealth`,0 AS `pag_ibig`,0 AS `nontax_income`,(case when (`pct`.`transaction_code` = 'REGND') then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `nd`,(case when (`pct`.`transaction_code` = 'REGOT') then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `reg`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_x8`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `sp`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `sp_x8`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_sp`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_sp_x8`,round((((aes_decrypt(`pp`.`salary`,`encryption_key`()) * 12) / `pp`.`total_year_days`) / 8),2) AS `hourly_rate`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `legal`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `legal_x8`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_legal`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_leg_x8`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'REGOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `nd_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `reg_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_x8_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `sp_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `sp_x8_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_sp_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_sp_x8_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `legal_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `legal_x8_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT_ND')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_legal_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `rest_leg_x8_otnd`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT_ND_EXCESS')) then round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) else 0 end) AS `nd_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'REGOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `reg_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'RDOT_EXCESS')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_x8_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `sp_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPEOT_EXCESS')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `sp_x8_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_sp_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'SPERDOT_EXCESS')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_sp_x8_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `legal_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGOT_EXCESS')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `legal_x8_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_legal_hrs`,(case when ((`ptc`.`transaction_class_code` = 'OVERTIME') and (`pct`.`transaction_code` = 'LEGRDOT_EXCESS')) then round(aes_decrypt(`pct`.`quantity`,`encryption_key`()),2) else 0 end) AS `rest_leg_x8_hrs`,0 AS `health_card`,0 AS `other_deduction_one`,0 AS `other_deduction_two`,0 AS `other_deduction_three`,0 AS `sss_sal_loan_payments`,0 AS `sss_cal_loan_payments`,0 AS `hdmf_sal_loan_payments`,0 AS `hdmf_cal_loan_payments`,0 AS `company_loan_payments`,0 AS `sss_sal_loan_balance`,0 AS `sss_cal_loan_balance`,0 AS `hdmf_sal_loan_balance`,0 AS `hdmf_cal_loan_balance`,0 AS `tax_status`,0 AS `ytd_sss`,0 AS `ytd_philhealth`,0 AS `ytd_pag_ibig`,`pp`.`tin` AS `tin`,`pt`.`transaction_label` AS `transaction_label`,`ptc`.`transaction_class_code` AS `transaction_class_code`,`pt`.`transaction_code` AS `transaction_code`,(case when (`pct`.`transaction_code` in ('ABSENCES','ABSENCES_ADJ','LWOP','LWOP_ADJ')) then concat(aes_decrypt(`pct`.`quantity`,`encryption_key`()),' (',`getabsent`(`t`.`payroll_date`,`u`.`user_id`,`pct`.`transaction_code`),') ') else aes_decrypt(`pct`.`quantity`,`encryption_key`()) end) AS `qty`,round(aes_decrypt(`pct`.`amount`,`encryption_key`()),2) AS `amount`,`pct`.`transaction_type_id` AS `transaction_type_id`,(case when (`pct`.`transaction_type_id` in (1,2,6,7,8)) then 'Earnings' when (`pct`.`transaction_type_id` in (3,4,5)) then 'Deductions' when (`pct`.`transaction_code` = 'NETPAY') then 'Netpay' else '' end) AS `group`,(case when ((`ptc`.`transaction_class_code` in ('LOAN_AMORTIZATION','LOAN_INTEREST')) and (`ptc`.`government_mandated` = 0)) then 'Loan' when ((`ptc`.`government_mandated` = 1) or (`ptc`.`transaction_class_code` = 'WHTAX')) then 'government' when (`ptc`.`transaction_class_code` = 'OVERTIME') then 'overtime' when ((`ptc`.`transaction_class_code` in ('ABSENCES','DEDUCTION_LATE','DEDUCTION_UNDERTIME')) or (`pt`.`transaction_code` = 'LWOP')) then 'attnd_ded' when (`pt`.`transaction_type_id` = 8) then 'Bonus' when (`pt`.`transaction_type_id` in (2,6)) then 'Benefits' when ((`pt`.`transaction_type_id` = 1) and (`pt`.`transaction_id` <> 1)) then 'Earnings' when (`pt`.`transaction_id` = 1) then 'salary' when (`pt`.`transaction_code` = 'NETPAY') then 'Netpay' else (case when (`pt`.`transaction_type_id` in (3,4,5)) then 'Oth_ded' when (`pt`.`transaction_type_id` in (1,2,6,7,8)) then 'Oth_inc' else '' end) end) AS `type`,(case when (`ptc`.`transaction_class_code` = 'LOAN_AMORTIZATION') then `pct`.`record_id` else '' end) AS `record_id`,(case when (`ptc`.`transaction_class_code` = 'LOAN_AMORTIZATION') then round(aes_decrypt(`pl`.`beginning_balance`,`encryption_key`()),2) else 0 end) AS `beginning_balance`,(case when (`ptc`.`transaction_class_code` = 'LOAN_AMORTIZATION') then round(aes_decrypt(`pl`.`running_balance`,`encryption_key`()),2) else 0 end) AS `running_balance`,`t`.`payroll_date` AS `payroll_date`,`t`.`date_from` AS `date_from`,`t`.`date_to` AS `date_to`,concat(date_format(if((`ptc`.`transaction_class_code` = 'NETPAY'),`t`.`payroll_date`,`t`.`date_from`),'%Y-%m'),'-',date_format(`t`.`date_from`,'%d')) AS `date_from_applicable`,date_format(if((`ptc`.`transaction_class_code` = 'NETPAY'),last_day(`t`.`payroll_date`),`t`.`date_to`),'%Y-%m-%d') AS `date_to_applicable`,`tc`.`taxcode` AS `taxcode`,`getcompany_contact`(`pct`.`company_id`,'Phone') AS `phone_no`,`getcompany_contact`(`pct`.`company_id`,'Fax') AS `fax_no` from (((((((((((((((`ww_payroll_closed_transaction` `pct` join `ww_users` `u` on((`pct`.`employee_id` = `u`.`user_id`))) join `ww_partners` `p` on((`u`.`user_id` = `p`.`user_id`))) join `ww_partners_personal` `ppe` on((`p`.`partner_id` = `ppe`.`partner_id`))) join `ww_taxcode` `tc` on((`tc`.`taxcode_id` = `ppe`.`key_value`))) join `ww_users_profile` `up` on((`u`.`user_id` = `up`.`user_id`))) join `ww_users_position` `upos` on((`up`.`position_id` = `upos`.`position_id`))) join `ww_payroll_partners` `pp` on((`pp`.`user_id` = `u`.`user_id`))) join `ww_users_department` `d` on((`d`.`department_id` = `up`.`department_id`))) join `ww_users_company` `uc` on((`uc`.`company_id` = `pp`.`company_id`))) left join `ww_payroll_period` `t` on((`t`.`payroll_period_id` = `pct`.`period_id`))) left join `ww_payroll_transaction` `pt` on((`pt`.`transaction_id` = `pct`.`transaction_id`))) left join `ww_payroll_transaction_class` `ptc` on((`pt`.`transaction_class_id` = `ptc`.`transaction_class_id`))) left join `ww_payroll_partners_loan` `pl` on((`pl`.`partner_loan_id` = `pct`.`record_id`))) left join `ww_users_project` `uproj` on((`pct`.`project_id` = `uproj`.`project_id`))) left join `ww_payroll_rate_type` `prt` on((`pct`.`payroll_rate_type_id` = `prt`.`payroll_rate_type_id`))) where ((`pct`.`on_hold` = 0) and (`ppe`.`key` = 'taxcode')) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `payroll_sss_to_disk_report` AS (select `payroll_partners_contribution`.`lastname` AS `lastname`,`payroll_partners_contribution`.`firstname` AS `firstname`,`payroll_partners_contribution`.`middlename` AS `middlename`,`payroll_partners_contribution`.`year` AS `year`,`payroll_partners_contribution`.`month` AS `month`,(`payroll_partners_contribution`.`sss_emp` + `payroll_partners_contribution`.`sss_com`) AS `contribution`,`payroll_partners_contribution`.`sss_ecc` AS `ec`,`payroll_partners_contribution`.`user_id` AS `user_id`,`payroll_partners_contribution`.`department_id` AS `department_id`,`payroll_partners_contribution`.`branch_id` AS `branch_id`,`payroll_partners_contribution`.`branch` AS `branch`,`payroll_partners_contribution`.`company_id` AS `company_id`,`payroll_partners_contribution`.`company` AS `company`,`payroll_partners_contribution`.`company_code` AS `company_code`,`payroll_partners_contribution`.`co_sss` AS `co_sss`,`get_sbr_date`(`payroll_partners_contribution`.`user_id`,`payroll_partners_contribution`.`payroll_date`,'SSS_EMP') AS `doc_date`,`get_sbr_no`(`payroll_partners_contribution`.`user_id`,`payroll_partners_contribution`.`payroll_date`,'SSS_EMP') AS `doc_no`,concat(rpad('20',5,' '),(case when (locate('ñ',`payroll_partners_contribution`.`lastname`) <> 'false') then rpad(replace(`payroll_partners_contribution`.`lastname`,' *',''),21,' ') else rpad(replace(`payroll_partners_contribution`.`lastname`,' *',''),20,' ') end),(case when (locate('ñ',`payroll_partners_contribution`.`firstname`) <> 'false') then rpad(`payroll_partners_contribution`.`firstname`,21,' ') else rpad(`payroll_partners_contribution`.`firstname`,20,' ') end),(case when (`payroll_partners_contribution`.`middlename` <> '') then rpad(replace(`payroll_partners_contribution`.`middlename`,'.',''),1,'') else ' ' end),rpad(replace(`payroll_partners_contribution`.`sss_no`,'-',''),11,'0'),round((`payroll_partners_contribution`.`sss_emp` + `payroll_partners_contribution`.`sss_com`),2),'000',round(`payroll_partners_contribution`.`sss_ecc`,2),lpad(if((`payroll_partners_contribution`.`govt_status` = 1),date_format(`payroll_partners_contribution`.`hired_date`,'%Y%m%d'),''),8,' '),`payroll_partners_contribution`.`govt_status`) AS `record`,now() AS `document_date` from `payroll_partners_contribution`) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `report_time_perfect_attendance` AS (select `p`.`id_number` AS `id_number`,`up`.`lastname` AS `lastname`,`up`.`firstname` AS `firstname`,year(`trs`.`date`) AS `year`,month(`trs`.`date`) AS `month_id`,`m`.`month` AS `month`,`up`.`company_id` AS `company_id`,`up`.`company` AS `company`,`ud`.`department_id` AS `department_id`,`ud`.`department` AS `department`,`up`.`branch_id` AS `branch_id`,`b`.`branch` AS `branch`,`time_record_form_lwp`(`u`.`user_id`,year(`trs`.`date`),month(`trs`.`date`)) AS `sum_lwp_forms` from ((((((`ww_users` `u` join `ww_partners` `p` on((`u`.`user_id` = `p`.`user_id`))) join `ww_users_profile` `up` on((`p`.`user_id` = `up`.`user_id`))) join `ww_time_record_summary` `trs` on((`u`.`user_id` = `trs`.`user_id`))) join `ww_users_department` `ud` on((`up`.`department_id` = `ud`.`department_id`))) left join `ww_users_branch` `b` on((`b`.`branch_id` = `up`.`branch_id`))) join `ww_month` `m` on((month(`trs`.`date`) = `m`.`month_id`))) where ((`trs`.`day_type` <> 'RESTDAY') and (`p`.`status_id` in (1,2)) and (`p`.`employment_type_id` in (5,6))) group by `u`.`user_id`,year(`trs`.`date`),month(`trs`.`date`) having ((sum(`trs`.`lwop`) = 0) and (`sum_lwp_forms` = 0) and (sum(`trs`.`late`) = 0) and (sum(`trs`.`undertime`) = 0) and (sum(`trs`.`absent`) = 0)) order by `up`.`company`,`ud`.`department`,`up`.`lastname`,`up`.`firstname`) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `time_form_code` AS (select `tr`.`date` AS `date`,`tf`.`form_code` AS `form_code`,`tr`.`user_id` AS `user_id` from ((`ww_time_record` `tr` join `ww_time_forms` `tf` on((`tr`.`user_id` = `tf`.`user_id`))) join `ww_time_forms_date` `tfd` on(((`tf`.`forms_id` = `tfd`.`forms_id`) and (`tr`.`date` = `tfd`.`date`))))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `time_forms_admin` AS (select `tf`.`forms_id` AS `forms_id`,`tf`.`form_status_id` AS `form_status_id`,`status`.`form_status` AS `form_status`,`tf`.`form_id` AS `form_id`,`tf`.`form_code` AS `form_code`,`form`.`form` AS `form`,`tf`.`reason` AS `reason`,`tf`.`user_id` AS `user_id`,`tf`.`display_name` AS `display_name`,`tf`.`day` AS `day`,`tf`.`hrs` AS `hrs`,concat(date_format(`tf`.`date_from`,'%b-%e %a'),if((`tf`.`date_from` = `tf`.`date_to`),'',concat(' To ',date_format(`tf`.`date_to`,'%b-%e %a')))) AS `date_range`,`tf`.`date_from` AS `date_from`,`tf`.`date_to` AS `date_to`,`gettimeline`(`tf`.`created_on`) AS `createdon`,`tf`.`created_on` AS `created_on`,if((`tf`.`user_id` = 0),`tf`.`form_status_id`,`approver`.`form_status_id`) AS `approver_status_id`,if((`tf`.`user_id` = 0),`status`.`form_status`,`approver_status`.`form_status`) AS `approver_status`,`approver`.`user_id` AS `approver_id`,`approver`.`display_name` AS `approver_name`,`tf`.`deleted` AS `deleted`,`tf`.`type` AS `type` from ((((`time_forms` `tf` left join `ww_time_forms_approver` `approver` on((`approver`.`forms_id` = `tf`.`forms_id`))) join `ww_time_form_status` `status` on((`status`.`form_status_id` = `tf`.`form_status_id`))) join `ww_time_form` `form` on((`form`.`form_id` = `tf`.`form_id`))) left join `ww_time_form_status` `approver_status` on((`approver_status`.`form_status_id` = `approver`.`form_status_id`))) where ((`tf`.`form_status_id` > 1) and (`tf`.`deleted` = 0)) group by `tf`.`forms_id`) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `time_forms_leave_type_filter` AS (select `ww_time_form`.`form_id` AS `form_id`,`ww_time_form`.`form_code` AS `form_code`,`ww_time_form`.`form` AS `form` from `ww_time_form` where ((`ww_time_form`.`form_code` in ('SL','VL','SIL')) and (`ww_time_form`.`deleted` = 0))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `time_forms_manage` AS (select `tf`.`forms_id` AS `forms_id`,`tf`.`form_status_id` AS `form_status_id`,`status`.`form_status` AS `form_status`,`tf`.`form_id` AS `form_id`,`tf`.`form_code` AS `form_code`,`form`.`form` AS `form`,`tf`.`reason` AS `reason`,`tf`.`user_id` AS `user_id`,`tf`.`display_name` AS `display_name`,`tf`.`day` AS `day`,`tf`.`hrs` AS `hrs`,concat(date_format(`tf`.`date_from`,'%b-%e %a'),if((`tf`.`date_from` = `tf`.`date_to`),'',concat(' To ',date_format(`tf`.`date_to`,'%b-%e %a')))) AS `date_range`,`tf`.`date_from` AS `date_from`,`tf`.`date_to` AS `date_to`,`gettimeline`(`tf`.`created_on`) AS `createdon`,`tf`.`created_on` AS `created_on`,`approver`.`form_status_id` AS `approver_status_id`,`approver_status`.`form_status` AS `approver_status`,`approver`.`user_id` AS `approver_id`,`approver`.`display_name` AS `approver_name`,`up`.`company_id` AS `company_id` from (((((`time_forms` `tf` join `ww_time_forms_approver` `approver` on((`approver`.`forms_id` = `tf`.`forms_id`))) join `ww_time_form_status` `status` on((`status`.`form_status_id` = `tf`.`form_status_id`))) join `ww_time_form` `form` on((`form`.`form_id` = `tf`.`form_id`))) join `ww_users_profile` `up` on((`up`.`user_id` = `tf`.`user_id`))) join `ww_time_form_status` `approver_status` on((`approver_status`.`form_status_id` = `approver`.`form_status_id`))) where ((`tf`.`form_status_id` > 1) and (`tf`.`deleted` = 0))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `time_forms_sl_vl_month` AS (select `u`.`user_id` AS `user_id`,`m`.`month_id` AS `month_id`,`t`.`form_code` AS `form_code` from ((`ww_users` `u` join `ww_month` `m`) join `ww_time_form` `t`) where (`t`.`form_code` in ('SL','VL'))) */;
